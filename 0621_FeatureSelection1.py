...
class NIDSEnv(gym.Env):
    ...
    def reset(self, seed=None, options=None):
        ...
        if not self.is_test:
            ...
            # Test 모드일 때는 테스트셋 내 실제 비율과 흐름 반영
...
def load_and_split_data(file_path):
    ...
    # 층화 추출(Stratified Split)로 오리지널 비율 고수하며 분리
...
if __name__ == "__main__":
    ...
    # [STAGE 1] 모델 학습 (Train)
    # ------------------------------------------
    print("\n========= [STAGE 1] DQN 기반 NIDS 특성압축 학습 시작 =========")
    for episode in range(num_train_episodes):
        ...
        for step in range(train_env.max_steps):
            ...
            next_state, reward, terminated, Network_truncated, info = train_env.step(action)
            done = terminated or Network_truncated
    ...
    for episode in range(num_test_episodes):
        ...
        for step in range(test_env.max_steps):
            action = agent.select_action(state)  
            next_state, reward, terminated, Network_truncated, info = test_env.step(action)
            done = terminated or Network_truncated
